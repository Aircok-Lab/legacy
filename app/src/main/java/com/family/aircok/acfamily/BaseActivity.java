...
import com.family.aircok.acfamily.common.widget.CXProgressDialog;
...
//			btn_dialog_left.setOnClickListener(listener);
	...
	public void setHeader(int left, int titleId, int title, int right, int titleColor) {
		...
		if (header != null) {
			...
			if (titleColor != -1){
				TextView tv_header_title = (TextView) header.findViewById(R.id.tv_header_title);
				tv_header_title.setVisibility(View.VISIBLE);
				tv_header_title.setTextColor(titleColor);
			}
		}
	}

	public void setNewHeader(int left, int leftnum, int title, boolean right, int titleColor) {
		View header = findViewById(R.id.layout_header);
		if (header != null) {
			if (left != -1) {
				ImageButton btn_header_left_icon = (ImageButton) header.findViewById(R.id.btn_header_left_icon);
				if (left == 0){
					btn_header_left_icon.setVisibility(View.GONE);
				} else {
					btn_header_left_icon.setVisibility(View.VISIBLE);
					btn_header_left_icon.setImageResource(left);
				}
			...
			if (title != -1) {
				header.findViewById(R.id.tv_title).setVisibility(View.VISIBLE);
				header.findViewById(R.id.postion_title).setVisibility(View.GONE);

				TextView tv_title = (TextView) header.findViewById(R.id.tv_title_txt);
				if (title == 0){
					tv_title.setText("");
				}else {
			...
			if (!right){
				header.findViewById(R.id.nav_batterry).setVisibility(View.GONE);
				header.findViewById(R.id.btn_nav_share).setVisibility(View.GONE);
			} 

			if (titleColor != -1){
				TextView tv_header_title = (TextView) header.findViewById(R.id.tv_title_txt);
				tv_header_title.setTextColor(titleColor);
			}
