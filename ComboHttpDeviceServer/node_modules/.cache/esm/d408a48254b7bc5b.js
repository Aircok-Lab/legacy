let PM25;_54b‍.x([["getPM25Score",()=>getPM25Score],["setPM25Score",()=>setPM25Score],["getPM25Alarm",()=>getPM25Alarm],["setPM25Alarm",()=>setPM25Alarm]]);_54b‍.w("../public/javascripts/defined",[["PM25",["PM25"],function(v){PM25=v}]]);

var IndexTable = require("../models/IndexTable");
var AlarmTable = require("../models/AlarmTable");
var global = require("../global");

       const getPM25Score = () => {
  console.log("getPM25Score 호출됨.");

  IndexTable.getIndexTableBySensorType(PM25, function(err, sensorIndexInfo) {
    if (err) {
      _54b‍.g.console.error(
        "getIndexTableBySensorType 처리 중 오류 발생 :" + err.stack
      );
      return;
    }

    //결과 객체 있으면 성공 응답 전송
    if (sensorIndexInfo) {
      global.sensorTable.pm25 = sensorIndexInfo;
    } else {
      console.error("PM25 : sensorIndexInfo 정보없음");
    }
  });
};

       const setPM25Score = (grade, min, max) => {
  _54b‍.g.console.log("setScore 호출됨 : " + grade + "," + min + "," + max);
  IndexTable.updateIndexTable(PM25, grade, min, max, function(
    err,
    sensorIndexInfo
  ) {
    if (err) {
      _54b‍.g.console.error("updateIndexTable 처리 중 오류 발생 :" + err.stack);
      return;
    }

    if (sensorIndexInfo) {
      global.sensorTable.pm25[grade - 1].min = min;
      global.sensorTable.pm25[grade - 1].max = max;
    } else {
      console.error("PM25 : sensorIndexInfo 정보없음");
    }
  });
};

       const getPM25Alarm = () => {
  console.log("getPM25Alarm 호출됨.");

  AlarmTable.getAlarmBySensorType(PM25, function(err, sensorAlarmInfo) {
    if (err) {
      _54b‍.g.console.error("getAlarmBySensorType 처리 중 오류 발생 :" + err.stack);
      return;
    }

    //결과 객체 있으면 성공 응답 전송
    if (sensorAlarmInfo) {
      global.alarm.pm25 = sensorAlarmInfo;
    } else {
      console.error("PM25 : sensorAlarmInfo 정보없음");
    }
  });
};

       const setPM25Alarm = value => {
  _54b‍.g.console.log("setScore 호출됨 : " + value);
  AlarmTable.updateAlarmValue(PM25, value, function(err, sensorAlarmInfo) {
    if (err) {
      _54b‍.g.console.error("updateAlarmTable 처리 중 오류 발생 :" + err.stack);
      return;
    }

    if (sensorAlarmInfo) {
      global.alarm.pm25 = value;
    } else {
      console.error("PM25 : sensorAlarmInfo 정보없음");
    }
  });
};
