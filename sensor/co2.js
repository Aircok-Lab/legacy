...
export const setCO2Score = (grade, min, max) => {
    ...
    IndexTable.updateIndexTable(CO2, grade, min, max, function(err, sensorIndexInfo){
        if(err){
        ...
        if(sensorIndexInfo){
            ...
            console.error('global.sensorTable.co2[grade-1].Min: ' + global.sensorTable.co2[grade-1].Min);
        } else {
            console.error('CO2 : sensorIndexInfo 정보없음');
        }
    });
...
export const setCO2Alarm = (value) => {
    ...
    IndexTable.updateAlarmTable(CO2, value, function(err, sensorAlarmInfo){
        if(err){
