...
export const setVOCScore = (grade, min, max) => {
    ...
    IndexTable.updateIndexTable(VOC, grade, min, max, function(err, sensorIndexInfo){
        if(err){
        ...
        if(sensorIndexInfo){
            ...
            console.error('global.sensorTable.voc[grade-1].Min: ' + global.sensorTable.voc[grade-1].Min);
        } else {
            console.error('VOC : sensorIndexInfo 정보없음');
        }
    });
...
export const setVOCAlarm = (value) => {
    ...
    IndexTable.updateAlarmTable(VOC, value, function(err, sensorAlarmInfo){
        if(err){
