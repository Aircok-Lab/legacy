...
var E3Core = {
    getSensorIndex: function(sensorType, sensorValue){
        var cMax=0, cMin=0, sMax=0, sMin=0, score=0;
        var data = 0, alarm = 0;
        var result = {
            index : 0,
            alarm : false,
        ...
        result.score = ((sMax - sMin) / (cMax - cMin)) * (sensorValue - cMin) + sMin;
    ...
    getTempIndex: function(temp){
        ...
        cOpt=(cMin+cMax)/2;
        result.score = 100 * (Math.abs(temp-cOpt) / (cMax - cMin))
    ...
    getHumidityIndex: function(humidity){
        ...
        cOpt=(cMin+cMax)/2;
        result.score = 100 * (Math.abs(humidity-cOpt) / (cMax - cMin))
        ...
        if((alarm - 10) >= humidity || humidity >= (alarm + 10))
            result.alarm = true;
    ...
    calTotalIndex: function(buildingType, pm10, pm25, co2, hcho, voc, temp, humi){
        ...
        switch(buildingType){
            case NEW_KINDERGARTEN:
                result.score = (0.1 * pm10 + 0.1 * pm25 + 0.3 * voc + 0.2 * hcho + 0.1 * co2) + (0.1 * temp + 0.1 * humi);
                break;
            case  KINDERGARTEN:
                result.score = (0.2 * pm10 + 0.3 * pm25+ 0.1 * voc + 0.1 * hcho + 0.1 * co2) + (0.1 * temp + 0.1 * humi);
                break;
            case NEW_POSTPARTUM:
                result.score = (0.1 * pm10 + 0.1 * pm25+ 0.3 * voc + 0.1 * hcho) + (0.2 * co2 + 0.1 * temp + 0.1 * humi);
                break;
            case POSTPARTUM:
                result.score = (0.15 * pm10 + 0.25 * pm25+ 0.1 * voc + 0.1 * hcho) + (0.2 * co2 + 0.1 * temp + 0.1 * humi);
                break;
            case NEW_OFFICE:
            if(month > 2 && month <  6 ){
                result.score = (0.1 * pm10 + 0.1 * pm25+ 0.1 * voc + 0.1 * hcho) + ( 0.4 * co2 + 0.1 * temp + 0.1 * humi);
            } else if(month > 5 && month <  9 ){
                result.score = (0.1 * pm10 + 0.1 * pm25+ 0.05 * voc + 0.05 * hcho) + ( 0.4 * co2 + 0.15 * temp + 0.15 * humi);
            } else if(month > 8 && month <  12 ){
                result.score = (0.1 * pm10 + 0.1 * pm25+ 0.1 * voc + 0.1 * hcho) + ( 0.4 * co2 + 0.1 * temp + 0.1 * humi);
            } else{
                result.score = (0.1 * pm10 + 0.1 * pm25+ 0.05 * voc + 0.05 * hcho) + ( 0.4 * co2 + 0.15 * temp + 0.15 * humi);
            ...
            case OFFICE:
                if(month > 2 && month <  6 ){
                    result.score = (0.1 * pm10 + 0.1 * pm25+ 0.15 * voc + 0.15 * hcho) + ( 0.4 * co2 + 0.05 * temp + 0.05 * humi);
                } else if(month > 5 && month <  9 ){
                    result.score = (0.1 * pm10 + 0.1 * pm25+ 0.1 * voc + 0.1 * hcho) + ( 0.4 * co2 + 0.1 * temp + 0.1 * humi);
                } else if(month > 8 && month <  12 ){
                    result.score = (0.1 * pm10 + 0.1 * pm25+ 0.15 * voc + 0.15 * hcho) + ( 0.4 * co2 + 0.05 * temp + 0.05 * humi);
                ...
                break;
            case NEW_HOUSE:
                result.score = (0.1 * pm10 + 0.3 * pm25+ 0.2 * voc + 0.1 * hcho) + ( 0.1 * co2 + 0.1 * temp + 0.1 * humi);
                break;
            case HOUSE:
                result.score = (0.1 * pm10 + 0.3 * pm25+ 0.2 * voc + 0.1 * hcho) + ( 0.1 * co2 + 0.1 * temp + 0.1 * humi);
                break;
