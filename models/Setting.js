...
var Setting = {
    getSettingById:function(id, callback){
        ...
        pool.getConnection(function(err, conn){
            ...
            // SQL문을 실행합니다.
            var exec = conn.query('Select * from Setting where id=?', id, function(err, result){
                conn.release(); // 반드시 해제해야 합니다.
    ...
    addSetting:function(scrollRow, scrollTime, monitoringTime, callback){
        ...
        pool.getConnection(function(err, conn){
            ...
            // 데이터를 객체로 만듭니다.
            var data = {ScrollRow:scrollRow, ScrollTime:scrollTime, MonitoringTime:monitoringTime};

            // SQL문을 실행합니다.
            var exec = conn.query('insert into Setting set ?', data, function(err, result){
                conn.release(); // 반드시 해제해야 합니다.
                console.log('실행 대상 SQL : ' + exec.sql);
    ...
    updateSetting:function(id, scrollRow, scrollTime, monitoringTime, callback){
        ...
        pool.getConnection(function(err, conn){
            ...
            // SQL문을 실행합니다.
            var exec = conn.query('update Setting set ScrollRow=?, ScrollTime=?, MonitoringTime=? where id=?', data, function(err, result){
                conn.release(); // 반드시 해제해야 합니다.
                console.log('실행 대상 SQL : ' + exec.sql);
