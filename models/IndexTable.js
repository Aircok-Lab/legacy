...
var IndexTable = {
    getIndexTableBySensorType:function(sensorType, callback){
        ...
        pool.getConnection(function(err, conn){
            ...
            // SQL문을 실행합니다.
            var exec = conn.query('Select * from IndexTable where SensorType=?', sensorType, function(err, result){
                conn.release(); // 반드시 해제해야 합니다.
    ...
    addIndexTable:function(sensorType, grade, min, max, callback){
        ...
        pool.getConnection(function(err, conn){
            ...
            // SQL문을 실행합니다.
            var exec = conn.query('insert into IndexTable set ?', data, function(err, result){
                conn.release(); // 반드시 해제해야 합니다.
                console.log('실행 대상 SQL : ' + exec.sql);
    ...
    updateIndexTable:function(sensorType, grade, min, max, callback){
        ...
        pool.getConnection(function(err, conn){
            ...
            // SQL문을 실행합니다.
            var exec = conn.query('update IndexTable set Min=?, Max=? where SensorType=? and Grade=?', data, function(err, result){
                conn.release(); // 반드시 해제해야 합니다.
                console.log('실행 대상 SQL : ' + exec.sql);
