...
var User = {
  ...
  loginUser2: function(loginId, password, callback) {
    console.log("loginUser2 호출됨");
    ...
    pool.getConnection(function(err, conn) {
      ...
      var loginUser = null;
      var queryString =
        "select UserPw.*, group_concat(distinct UserBuilding.buildingID) as buildingList, \
        group_concat(distinct UserPosition.positionID) as positionList, \
        group_concat(distinct UserDevice.deviceID) as deviceList \
        from UserPw, UserBuilding, UserPosition, UserDevice";
      queryString =
        queryString +
        " where UserPw.loginID = ?"; 
      // 패스워드비교안함.
      //  " and User.password = AES_ENCRYPT(?, SHA2('key', 512))";
        " and UserPw.password = ?"; 
      queryString =
        queryString +
        " and UserPw.id = UserBuilding.userID and UserPw.id = UserPosition.userID and \
        UserPw.id = UserDevice.userID group by UserPw.id";
