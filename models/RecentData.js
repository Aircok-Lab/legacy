...
var RecentData = {
  ...
  getRecentDataByDeviceSN: function(deviceSN, callback) {
    ...
    pool.getConnection(function(err, conn) {
      ...
      var queryString =
        "select Building.name as buildingName , Position.name as positionName, Device.name as deviceName, Device.predict_yn as predict_yn, Building.latitude as latitude, Building.longitude as longitude, RecentData.*, \
  ...
  updateRecentData: function(
    ...
    pool.getConnection(function(err, conn) {
      ...
      var exec = conn.query(
        ...
        // 'and date <= DATE_FORMAT(NOW(), "%Y-%c- %d %T")",
        data,
        function(err, result) {
          conn.release(); // 반드시 해제해야 합니다.
          console.log("실행 대상 SQL : " + exec.sql);
