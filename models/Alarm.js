...
var Alarm = {
    ...
    addAlarm:function(pm10, pm25, co2, hcho, voc, temperature, humidity, noise, total, date, deviceSN, callback){
    ...
    updateAlarm:function(id, pm25, pm10, co2, hcho, voc, temperature, humidity, noise, date, deviceSN, callback){
        ...
        pool.getConnection(function(err, conn){
            ...
            // SQL문을 실행합니다.
            var exec = conn.query('update '+tableName+' set PM25=?, PM10=?, CO2=?, HCHO=?, VOC=?, Temperature=?, Humidity=?, Noise=?, Date=?, DeviceSN=? where id=?', data, function(err, result){
                conn.release(); // 반드시 해제해야 합니다.
                console.log('실행 대상 SQL : ' + exec.sql);
