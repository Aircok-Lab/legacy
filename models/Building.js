...
var Building = {
    ...
    addBuilding:function(name, address, latitude, longitude, callback){
        ...
        pool.getConnection(function(err, conn){
            ...
            // 데이터를 객체로 만듭니다.
            var data = {Name:name, Address:address, Latitude:latitude, Longitude:longitude};

            // SQL문을 실행합니다.
            var exec = conn.query('insert into Building set ?', data, function(err, result){
                conn.release(); // 반드시 해제해야 합니다.
                console.log('실행 대상 SQL : ' + exec.sql);
    ...
    updateBuilding:function(id, name, address, latitude, longitude, callback){
        ...
        pool.getConnection(function(err, conn){
            ...
            // SQL문을 실행합니다.
            var exec = conn.query('update Building set Name=?, Address=?, Latitude=?, Longitude=? where id=?', data, function(err, result){
                conn.release(); // 반드시 해제해야 합니다.
                console.log('실행 대상 SQL : ' + exec.sql);
