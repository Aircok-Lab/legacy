...
var Product = {
  ...
  getProductBySerialNumber: function(serialNumber, callback) {
    console.log("getProductBySerialNumber 호출됨");
    ...
    pool.getConnection(function(err, conn) {
      ...
      // SQL문을 실행합니다.
      var exec = conn.query(
        "select Product.* from Device, Product where  Device.productID = Product.id and Device.serialNumber = ?",
        serialNumber,
        function(err, result) {
          conn.release(); // 반드시 해제해야 합니다.
          console.log("실행 대상 SQL : " + exec.sql);
  ...
  updateProduct: function(
    ...
    pool.getConnection(function(err, conn) {
      ...
      // SQL문을 실행합니다.
      var exec = conn.query(
        "update Product set name=?, version=?, ip=?, period=?, indoor=?, firmware=?, filesize=?, pm25=?, pm10=?, co2=?, hcho=?, voc=?, temperature=?, humidity=?, noise=?, co=? where id=?",
        data,
        function(err, result) {
          conn.release(); // 반드시 해제해야 합니다.
          console.log("실행 대상 SQL : " + exec.sql);
