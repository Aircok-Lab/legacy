...
import AdviceInfo1 from "./adviceInfo1.js";
...
class DetailPage extends React.Component {
  ...
  componentDidMount() {
    ...
    this.intervalScrollHandle = setInterval(
      this.scrollDevice,
      this.props.data.monitoringTime * 10000
    );
  }
    ...
    return (
      ...
      >
        <div>
          ...
          <div className="card-deck mx-3 mb-3" style={{marginTop:"35px" }}>
            {contact ? (
              <DeviceInfo
                time={contact.date}
                buildingName={contact.buildingName}
            ...
            hcho = {contact.hcho}
            co2= {contact.co2}
            humidity= {contact.humidity}
            voc = {contact.voc}
            pm25= {contact.pm25}
            pm10= {contact.pm10}
            pm10Index = {contact.pm10Index}
            pm25Index= {contact.pm25Index}
            co2Index= {contact.co2Index}
            hchoIndex= {contact.hchoIndex}
            vocIndex= {contact.vocIndex}
            temperatureIndex= {contact.temperatureIndex}
            humidityIndex= {contact.humidityIndex}
            positionName= {contact.positionName}
            deviceName= {contact.deviceName}
            date= {contact.date}
            e3Index= {contact.e3Index}
            indoor= {contact.indoor}
              />
