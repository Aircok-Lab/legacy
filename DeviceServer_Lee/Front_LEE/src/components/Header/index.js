...
class Header extends React.Component {
  ...
  render() {
    ...
    return (
      ...
      >
        <Toolbar style={{backgroundColor:'#373743'}} className="app-toolbar" disableGutters={false}>
          {navigationStyle === HORIZONTAL_NAVIGATION ? (
            <div className="d-block d-md-none pointer mr-3" style={{backgroundColor:'black'}} onClick={this.onToggleCollapsedNav}>
              <span className="jr-menu-icon">
                <span className="menu-icon" />
              </span>
            </div>
          ) : (
            <IconButton className={`jr-menu-icon mr-3 ${drawerStyle}`} aria-label="Menu" style={{backgroundColor:"#595868",left:"-14px"}}  onClick={this.onToggleCollapsedNav}>
              <span className="menu-icon" />
            </IconButton>
          )}

          <Link className="app-logo mr-2 d-none d-sm-block" to="/">
            <img src="assets/icons/smart_aircok12.png" alt="smart aircok" title="Smart Aircok" style={{marginLeft: "-17px",marginTop: "-4px"}}/>
          </Link>
          <IndexHeader />
