...
router.post("/", function(req, res, next) {
  ...
  if (paramData) {
    ...
    Device.getDeviceInfo(paramDeviceSN, function(err, info) {
      ...
      if (info) {
          // timer 시간 적용 관련
          if(info.window_open == "0" || info.window_open == null)
          {            
            ...
            var check_time = Number(date);
            console.log("#################: TZ-Seoul time:"+date);    

            // var current_time = new Date();
            // var hour = current_time.getHours();
            // var min = current_time.getMinutes();
            // var check_time = Number(hour) * 100 + Number(min);
            // console.log("#################: time:"+check_time);


            if(check_time >= 630 && check_time < 700) info.window_open = 2;          // OPEN
            else if(check_time >= 1200 && check_time < 1230) info.window_open = 2;   
            else if(check_time >= 1025 && check_time < 1040) info.window_open = 2;
            //else if(check_time == 9999 ) info.window_open = 0; // NOTHING..
            else info.window_open = 1;                    // CLOSED..
            info.window_open = info.window_open.toString();                    // CLOSED..
            console.log("#################:Schedule - info.window_open:"+ info.window_open);
          }
          else console.log("#################:WebSetting - info.window_open:"+ info.window_open);
          ...
          // console.log("#################:3");
            ...
            if ( paramDate != "9999-99-99 99:99:00" ) {  
              ...
              // 공기청정기, 에어콘 가동 구분               
                  ...
                  "|" +
                  ir_temp +     // 신세계 테스트는 ir 이후는 없다.                  
                  "|" +
                  //
                  //

                  nvl(info.window_open,"null");     // 창문개폐 여부 0 - 폐, 1-개
console.log("info.window_open :"+info.version); 
                  //
                  //"$$aircok:e$$";                  
                  ...
                  // result = "$$aircok:s$$OK|"+paramDate+"$$aircok:e$$";  // 신세계 테스트 시는 이걸로 그냥 한다. 

                  console.log("최종전송 디바이스 정보 값1 :"+result); 
